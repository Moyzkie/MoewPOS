  ...
  <div class="modal-dialog">
   <div class="modal-content">
    <form action="" METHOD="POST" id="edit_admin" enctype="multipart/form-data">
     <div  style="" class="modal-header">      
      <h4 class="modal-title text-black" id="">Edit admin</h4>
      <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
     </div>
     <div class="modal-body row">     
      <div class="form-group col-md-12">
      ...
      <div class="form-group col-md-12">
       ...
       <select class="form-control" id="edit_status"  name="edit_status" required>
        ...
        <span id ="error_editstatus" class="alert-danger"></span>
      </div> 
      <div class="form-group col-md-12">
         <label>Profile</label>
     ...
     <div  style="" class="modal-footer">
     <input type="reset" class="btn btn-danger"  value="Clear">
      <button type="button"  onclick="editAdmin()"  class="btn btn-primary" >Save</button>
     </div>
    </form>
   </div>
  </div>
...
<script>
 $(document).ready(function(){
    ...
    function adminInfo(){
      $('#admin').DataTable({
      destroy: true,
      ajax:"<?=base_url('My_controller/displayAdminInfo')?>",
      columns:[
        {data:"id"},
        {data:"username"},
        {data:"email"},
        {data: null, title: 'Action', wrap: true, "render": function (item) { return '<div class="btn-group"><button type="button" class="btn btn-danger dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> Action</button><div class="dropdown-menu"> <a class="dropdown-item" onclick="getAdmin('+item.id+')" href="" data-toggle="modal" data-target="#editmodal">EDIT</a> <a class="dropdown-item" href="#"  onclick="deleteAdmin('+item.id+')">DELETE</a></div>' } }
      ],
...
$.post("<?=base_url('My_controller/getAdmin')?>",{
   id:id
...
function addAdmin(){
    ...
    $.ajax({
        url:"<?=base_url('My_controller/addAdmin')?>",
        type:"POST",
        data:formData,
        processData: false,
        contentType: false,
...
$.ajax({
    url:"<?=base_url('My_controller/editAdmin')?>",
    type:"POST",
    data:formData,
    processData: false,
    contentType: false,
...
function deleteAdmin(id){
       ...
       if(con == true){
          $.ajax({
            url:"<?=base_url('My_controller/deleteAdmin')?>",
            type:"POST",
            data: {id:id},
